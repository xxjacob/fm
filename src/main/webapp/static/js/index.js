		...
		var thumbBtnAction = function(thumb) {
			if (current) {
				$
						.ajax({
							async : false,
							url : "/fm/" + thumb + "?sid=" + current.song.id
									+ "&r=" + new Date().getTime(),
							success : function(json, statusCode) {
								if (json.err_no == 0) {
									...
									for ( var i in playLists) {
										...
										if (playList.type == DOWN
												&& json.thumbdown_num >= 0) {
											playList.songCount = json.thumbdown_num;
										}
										if (playList.type == TIRED
												&& json.tired_num >= 0) {
											playList.songCount = json.tired_num;
										}
										if (playList.element)
											$(playList.element)
													.find(".stationNameText")
													.html(
															playList.name
																	+ ' ['
																	+ playList.songCount
					...
					success : function(json, statusCode) {
						...
						if (json && json.length > 0) {
							for (var i in json){
								...
								if (ss[0]){
									var id = (ss[1]?ss[1]:0);
									var li = $('<li class="menu" data="'+id+'" ><a class="signoutMenuItem" href="#">'+ss[0] + (ss[2]?(' - ' + ss[2]):'')+'</a></li>');
									if (id > 0){
										li.click(function(e){
											e.preventDefault();
							...
							ul.append('<li class="menu"><a class="signoutMenuItem" href="#"><i>Sorry, Cannot Find "'+v+'". Upload?</i></a></li>');
						}
						searchRst.show();
