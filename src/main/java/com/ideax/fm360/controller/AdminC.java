...
public class AdminC {
	...
	// ----------------------------------------
	@RequestMapping("add")
	public String addSong(HttpServletRequest req) throws Exception {
		return "admin/index";
	}

	@RequestMapping("search")
	public String searchXiami(@RequestParam String songName, @RequestParam(defaultValue = "") String artist, Model model)
			throws Exception {
		String searchUrl = "http://www.xiami.com/search?key=" + songName.replace(" ", "+") + "+" + artist.replace(" ", "+");
		Document doc;
		try {
			doc = Jsoup.connect(searchUrl).get();
		} catch (IOException e1) {
			e1.printStackTrace();
			model.addAttribute("searchResult", "no search result : <a href='" + searchUrl + "'>" + searchUrl + "</a>");
